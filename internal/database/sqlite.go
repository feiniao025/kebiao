...
func (s *SQLite) migrate() error {
	...
	// 回填 period_count（老数据 NULL 或 0 都补成 8）
...
func (s *SQLite) DeleteUser(id int64) error {
	...
	for _, table := range []string{"schedule_cells", "students", "seat_configs", "user_preferences", "classes"} {
...
func (s *SQLite) GetSeatConfig(userID int64) (*model.SeatConfig, error) {
	...
	if err == sql.ErrNoRows {
		_, err = s.db.Exec(
			`INSERT INTO seat_configs (user_id, rows, cols, "order", aisle) VALUES (?, 7, 8, 'asc', '')`,
			userID,
		)
		if err != nil {
			return nil, err
